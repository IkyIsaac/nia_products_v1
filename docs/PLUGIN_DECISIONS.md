...
Current site state (verified via `wp-cli`, updated 2026-07-07 after Phase 1 environment work): WooCommerce installed and active with HPOS enabled; Rank Math, Wordfence, UpdraftPlus, Fluent Forms, FluentSMTP, the category-ordering plugin (`taxonomy-terms-order`), CookieYes (`cookie-law-info`), and ShortPixel all installed and active; Elementor (free) remains installed, scoped as described below; default theme `twentytwentyfive` is still active (Nia Theme scaffold is a Phase 2 task — only its build tooling exists so far). **WPML is not yet installed — blocked on a client-purchased license, see `RISKS.md` R20.** This section otherwise remains the plan/reasoning record, not a live inventory — check `wp plugin list` for the current authoritative state.
