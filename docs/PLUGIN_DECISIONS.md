...
Current site state (verified via `wp-cli`, updated 2026-07-07 after Phase 3 global-components work): WooCommerce installed and active with HPOS enabled; Rank Math, Wordfence, UpdraftPlus, Fluent Forms, FluentSMTP, the category-ordering plugin (`taxonomy-terms-order`), CookieYes (`cookie-law-info`), and ShortPixel all installed and active; Elementor (free) remains installed, scoped as described below, with its Global Colors/Fonts synced to `DESIGN_SYSTEM.md` (`ARCHITECTURE.md` §2a) and a 6-template Saved Template library built (Phase 3); the custom **Nia Theme is scaffolded and active**, replacing `twentytwentyfive` (compiled Tailwind, self-hosted fonts, base templates, real header/footer/mobile-nav, 8 button variants, card system — see `PROJECT_PLAN.md` Phase 2/3); the custom **Nia Core plugin is now scaffolded and active** (`wp-content/plugins/nia-core`), currently holding the 5 custom Gutenberg blocks (`class-nia-blocks.php`) — the business-logic classes described below (Selcom gateway, subscriptions, WhatsApp, etc.) are still future phases. **WPML is not yet installed — blocked on a client-purchased license, see `RISKS.md` R20.** This section otherwise remains the plan/reasoning record, not a live inventory — check `wp plugin list`/`wp theme list` for the current authoritative state.
