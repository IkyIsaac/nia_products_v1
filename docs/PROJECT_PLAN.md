...
**Exit criteria:** side-by-side comparison against each mockup shows matching layout, type, color, and spacing; responsive behavior verified at mobile/tablet/desktop. **Met for layout/type/color/spacing on Shop + PDP**, now including real-browser verification (Playwright screenshots at 390px/1440px), not just curl. **Not yet independently verified:** the same real-browser pass for Cart/Checkout — their client-rendered (Interactivity API) content is still only curl/Store-API-verified, and is the top recommended follow-up before calling this phase fully signed off. **Also outstanding (content gap, not a build task):** 3 of the 8 dummy products ("Raw Seamoss — 200g," "Seamoss Powder — 250g," "Seamoss Powder — 500g") have no featured image assigned and show an empty placeholder box on the Shop grid.
