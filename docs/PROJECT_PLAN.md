...
- **Create the 4 initial product categories** (Seamoss Powder, Seamoss Gel, Raw Seamoss, Seamoss Capsules) via Products → Categories, each with a thumbnail/description — **not 4 individual products** (corrected 2026-07-08; see `ARCHITECTURE.md` §1/§9).
- Create the initial product(s) within each category with correct data (price in TZS, descriptions, images) — however many the client actually has ready at launch; the count per category is not fixed and is expected to change.
- Install/configure the category-ordering plugin (see `PLUGIN_DECISIONS.md`) so the admin can control category display order from day one.
- Confirm with the client whether any category needs variable products (size/flavor variants within one listing) vs. separate simple products per variant — either is native WooCommerce, this is a data-entry decision, not an architecture one.
- Cart & Checkout: implement using WooCommerce's block-based Cart/Checkout (see `ARCHITECTURE.md` §6).
- My Account: enable and scope which default endpoints are needed (Orders, Downloads off, Addresses, Account details) plus the custom "My Rituals"/subscriptions view (ties to Phase 9).
- Shop archive / Collection page functional as a true category archive with pagination and a category filter/tab row (unstyled acceptable here; see `ARCHITECTURE.md` §9a, `DESIGN_SYSTEM.md` §10).
- Homepage "Featured Products" section wired to WooCommerce's native Featured-product flag, not hardcoded product IDs (see `ARCHITECTURE.md` §9a).
- Product detail page functional with the one-time/subscription purchase-type radio wired to price swap logic (JS/Alpine) — built as the one shared template for any product in any category.
- Verify WPML's WooCommerce Multilingual & Multicurrency add-on's multicurrency module works with the block-based Cart/Checkout and the custom Selcom gateway (see `PLUGIN_DECISIONS.md`); decide and document the exchange-rate source, update cadence, and rounding rule (see `ARCHITECTURE.md` §11, `RISKS.md` R14).
- Translate the initial categories and products (name, description, subscription-tier copy) into Kiswahili via the WooCommerce Multilingual add-on's translation editor — **and establish this as an ongoing part of the "add a category/product" workflow**, not a one-time task, since the catalog will keep changing (see `RISKS.md`).

**Exit criteria:** a full purchase can be completed end-to-end in test mode (browse → add to cart → checkout → order created), functionally correct, visual polish pending.

**Docs to update:** `CUSTOMIZATIONS.md` (log every hook/override added), `PAGE_STATUS.md`.
