...
**Status: complete (2026-07-08), extended with a real-browser UI/UX pass on the remaining unstyled pages (2026-07-22 to 2026-07-23).**
...
- [x] **UI/UX overhaul round (2026-07-22 to 2026-07-23)**, user-driven page-by-page: with the core flows in place, several pages turned out to have no real design applied at all — they'd been quietly falling back to WooCommerce's or the theme's raw default markup since nothing had built a template override or CSS for them yet. Fixed:
  - **Login / Register / Lost password / Reset password** (`myaccount/form-login.php`, `form-lost-password.php`, `form-reset-password.php`): none had a theme override before — WooCommerce rendered its unstyled default markup. Account registration was also disabled in WooCommerce settings (no way for a customer to sign up at all) — enabled it. Built to match the existing card/button convention, with the two Login/Register cards' primary buttons aligned to the same baseline via `mt-auto` regardless of each form's differing content length.
  - **Edit Account / Edit Address** (`myaccount/form-edit-account.php`, `form-edit-address.php`, `my-address.php`): same gap, same fix — real templates matching the dashboard convention, WooCommerce's own `form-row-first/last/wide` classes driving a 2-column field grid.
  - **My Account sidebar overlapping the footer**: the sidebar was `position:fixed` pinned to the viewport bottom, so it stayed painted over whatever scrolled underneath it — including the footer — regardless of page content height. Switched to `sticky`, confined to the account layout's own row.
  - **Shop/PDP product-card nested-anchor bug**: the whole card was wrapped in one `<a>`, with the Quick Add button's own `<a>` nested inside it — invalid HTML, which made the browser silently split the outer link and show an unlabeled phantom button on hover. Restructured so the product image is always a real clickable link (no hover needed to discover it), Quick Add now a proper sibling element.
  - **PDP Add to Bag row**: the quantity input was a bare, unstyled browser-default number field floating above the button — now one aligned row.
  - **Reviews section**: sort/rating-filter links caused a jarring scroll jump to the top of the section on every click (plain links were full page reloads landing on a URL hash) — converted to one AJAX refresh of the reviews grid (`history.pushState`, no reload). The star-rating picker (Write a Review) only recolored on hover/select instead of actually filling in solid like the average-rating display does — fixed.
  - **Checkout**: had no dedicated template, so it was stuck on `page.php`'s 768px container — content was visibly cut off, and that same width bug was independently causing WooCommerce's checkout block to render a duplicate order summary (its own mobile-vs-desktop breakpoint detection was tripping into "mobile" mode even on a full desktop viewport at 768px). New `page-checkout.php` fixes both at once. Also re-themed the checkout block's own default input/checkbox/focus-ring styling (separate from the classic WooCommerce stylesheet this theme already disables) to match the brand, and styled the trust row + payment note, which had no CSS at all before.
  - **Order-received ("thank you") page**: same "never had a template/CSS" gap — styled via WooCommerce's classic-template stable class names (not a full override), plus a new "Your Ritual" summary shown for subscription orders (cadence, products, first delivery date), hooked onto the same `woocommerce_thankyou` action the subscription-creation logic already uses, at a later priority, so it can display the exact records that were just created on that same page load.
  - **My Account → Order History**: no page heading, and the table had zero cell padding (rendered squeezed/illegible) — real pass: heading, cell padding + row hover, order status as a colored pill (matching the dashboard's Recent Orders preview) instead of plain text, "View" as a real link, a stacked-card layout on mobile (reusing the `data-title` attributes WooCommerce's own template already outputs but which nothing was using), and styled pagination/empty-state.
  - **Cart**: identical gap to Checkout (stuck on `page.php`'s 768px container, no card styling for the totals sidebar) — new `page-cart.php`, sidebar card treatment, styled item-table header row, and polish on the "Complete Your Ritual" cross-sell block's photo/price/button treatment.
  - **Subscription page**: the 3 tier cards were pure decoration (hardcoded prices, fabricated perks like "Monthly Wellness Workshop") with no logic behind "Subscribe Now" — now pull the real admin-set discount/term per cadence and open a working product picker wired to `Nia_Subscriptions` (see Phase 9).
  - **Recurring bug, found on Cart and fixed in 3 places**: the fixed header's logo wraps to two lines at common desktop widths and renders ~189px tall — taller than the `pt-32` (128px) or `py-section-gap` (120px) clearance `page-cart.php`, `page-checkout.php`, and the shared `page.php` (behind all 4 policy pages) assumed. Every one of those page titles was rendering completely invisible, painted over by the fixed header. Fixed in all three.
  - **Privacy Policy page cleanup**: `/privacy-policy/` was still WordPress's own empty default draft page (and was still the page configured in Settings → Privacy) — the real, fully-written policy lived at the auto-suffixed `/privacy-policy-2/` URL and was only reachable via a 301 redirect. Renamed the draft's slug out of the way, gave the real page the clean `/privacy-policy/` slug, and repointed the privacy settings — no content deleted, no redirect needed anymore.
  - **`NIA_THEME_VERSION` cache-busting**: had been hardcoded to `0.1.0` since Phase 2 and never bumped once despite `main.js`/`style.css` changing across many commits since — a returning visitor's browser could cache old JS indefinitely against that unchanging `?ver=` query string and silently keep running it after a fix shipped, with no error to show for it. Bumped, and this should be bumped on every future CSS/JS change.

**Exit criteria:** side-by-side comparison against each mockup shows matching layout, type, color, and spacing; responsive behavior verified at mobile/tablet/desktop. **Met for layout/type/color/spacing on Shop, PDP, Cart, and Checkout**, all four now real-browser verified (Playwright screenshots at 390px/1440px, not just curl). **Real-browser QA on Cart/Checkout (2026-07-21)** found and fixed four `input.css` selectors that didn't match WooCommerce Blocks' actual class names (see `PAGE_STATUS.md`/`CUSTOMIZATIONS.md` for the exact corrections) — a Tailwind content-purging issue was also found and fixed the same day: several component classes only ever appear inside `nia-core/includes/` (not previously scanned) or exclusively inside WooCommerce Blocks' own rendered markup (never in any scanned file at all), so Tailwind's JIT was silently dropping them from the compiled stylesheet; `tailwind.config.js` now scans the missing directory and safelists the WC-Blocks-only classes. **Real-browser QA extended to the account/auth flow (2026-07-22 to 2026-07-23)** — see the UI/UX overhaul round above; every page it covered is now real-browser verified at mobile + desktop with a real logged-in test account, cleaned up afterward. **Still outstanding (content gap, not a build task):** 3 of the 8 dummy products ("Raw Seamoss — 200g," "Seamoss Powder — 250g," "Seamoss Powder — 500g") have no featured image assigned and show an empty placeholder box on the Shop grid.
